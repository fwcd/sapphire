...
import com.fwcd.amethyst.utils.Vector3i;
...
public class Camera {
	...
	private void syncWithEntity() {
		if (trackedEntity != null) {
			...
			if (firstPerson) {
				...
				float hDist = thirdPersonDistToTrackedEntity * AmethystMath.cos(pitchDeg);
				float vDist = thirdPersonDistToTrackedEntity * AmethystMath.sin(pitchDeg);
				float offsetX = hDist * AmethystMath.sin(theta);
				float offsetZ = hDist * AmethystMath.cos(theta);
	...
	public void moveRight(float delta) {
		float dx = AmethystMath.cos(yawDeg) * delta;
		float dz = AmethystMath.sin(yawDeg) * delta;
		translate(dx, 0, dz);
	}
	...
	public void moveForward(float delta) {
		float dx = AmethystMath.cos(yawDeg - 90) * delta;
		float dz = AmethystMath.sin(yawDeg - 90) * delta;
		translate(dx, 0, dz);
	}
